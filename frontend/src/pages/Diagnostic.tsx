...
const Diagnostic = () => {
  ...
  const sortedOpportunities = [...filteredOpportunities].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'cost-high':
        return b.upfrontCost.max - a.upfrontCost.max;
      case 'effort-low':
        const effortOrder = { 'DIY': 1, 'Professional': 2, 'Specialist': 3 };
        return (effortOrder[a.difficulty as keyof typeof effortOrder] || 2) -
               (effortOrder[b.difficulty as keyof typeof effortOrder] || 2);
      case 'effort-high':
        const effortOrderDesc = { 'DIY': 1, 'Professional': 2, 'Specialist': 3 };
        return (effortOrderDesc[b.difficulty as keyof typeof effortOrderDesc] || 2) -
               (effortOrderDesc[a.difficulty as keyof typeof effortOrderDesc] || 2);
      case 'default':
      default:
        // Default: Sort by cost tier (free first), then by annual savings
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1">
            ...
            <select
              ...
              className="border border-gray-300 rounded-md px-3 py-2 w-full sm:w-auto"
            >
              <option value="default">Default (Free First)</option>
              <option value="savings-high">Savings (High to Low)</option>
              <option value="savings-low">Savings (Low to High)</option>
              <option value="cost-low">Cost (Low to High)</option>
              <option value="cost-high">Cost (High to Low)</option>
              <option value="effort-low">Effort (Low to High)</option>
              <option value="effort-high">Effort (High to Low)</option>
            </select>
          </div>
        </div>
